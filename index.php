...
$progadmin = false;
$prog = false;
$report_heading = get_string('dashboard', 'report_myfeedback');
...
if (user_has_role_assignment($USER->id, $p_tutor_id)) {
    $personal_tutor = true;
    $report_heading = get_string('dashboard', 'report_myfeedback') . get_string('moduleleaderview', 'report_myfeedback');
}
...
if (user_has_role_assignment($USER->id, $prog_admin_id)) {
    $prog = true;
    $report_heading = get_string('dashboard', 'report_myfeedback') . get_string('progadminview', 'report_myfeedback');
}
...
if ($mods = enrol_get_users_courses($userid, $onlyactive = TRUE)) {
    foreach ($mods as $value) {
        ...
        if (has_capability('report/myfeedback:progadmin', $coursecontext, $USER->id, $doanything = false)) {//&& user_has_role_assignment($userid, 5, $coursecontext->id)){
            $progadmin = true;
            $report_heading = get_string('dashboard', 'report_myfeedback') . get_string('progadminview', 'report_myfeedback');
        }
    }
...
if ((!$module_tutor && !$personal_tutor && !$prog) || $viewtutee) {
    ...
    //for ($i = 1; $i <= $years; $i++) {
    //    $tabs[] = new tabobject('Year ' . $i, new moodle_url($thispageurl, array('userid' => $userid, 'currenttab' => 'Year ' . $i)), 'Year ' . $i);
    //}
...
if (($module_tutor || $personal_tutor) && !$viewtutee) {
...
switch ($currenttab) {
    ...
    case 'Year 5':
    case 'Year 6':
        require_once('student/yearlevel.php');
        break;
    case 'feedback':
