 ...
 * @credits   Based on original work report_mygrades by David Bezemer <david.bezemer@uplearning.nl> which in turn is based on block_myfeedback by Karen Holland, Mei Jin, Jiajia Chen. Also uses SQL originating from Richard Havinga <richard.havinga@ulcc.ac.uk>.
...
// Use a custom $remotedb (and not current system's $DB) if set - code sourced from configurable reports plugin
...
echo $OUTPUT->heading(get_string('pluginname', 'report_myfeedback')." ".get_string('for', 'calendar')." ".$userlinked);
...
class report_myfeedback extends block_base {
	...
	* @return bool true if the module exists and is not hidden in the site admin settings, otherwise false
	*/
	public function mod_is_available($modname){
		global $remotedb;

		$installed_plugins = core_plugin_manager::instance()->get_plugins_of_type('mod');
		
		//is the module installed?
		if(array_key_exists($modname, $installed_plugins)){
	...
	* @return str Any comments left by a marker on a Turnitin Assignment via the Moodle Comments feature (not in Turnitin), each on a new line
	*/
	public function get_quiz_attempts_link($quizid, $userid, $quizurlid){
		...
		if($attemptcount > 0){
			...
			$attemptstext = ($attemptcount > 1)?get_string('attempts', 'report_myfeedback'):get_string('attempt', 'report_myfeedback');
			$out[] = html_writer::link($url, get_string('review', 'report_myfeedback') . " ". $attemptcount." ".$attemptstext);
		}
	...
	/**
	* Get group assignment submission date - since it won't come through for a user in a group unless they were the one's to upload the file
	* 
	* @param int  $userid The id of the user
	...
	* 
	* @return str The table of submission and feedback for the user refrred to in the url after userid=
	*/
	public function get_content() {
		...
		if($this->mod_is_available("assign")){
			$sql .=    "SELECT c.id AS courseid,
						...
						FROM {course} c
						JOIN {grade_items} gi ON c.id=gi.courseid AND itemtype='mod' AND (gi.hidden = 0 or gi.hidden < unix_timestamp(current_timestamp))
						...
						JOIN mdl_assign_user_flags auf ON a.id = auf.assignment AND auf.workflowstate = 'released' AND auf.userid = ? OR a.markingworkflow = 0
		...
		//these are the most params needed, but depending on how many modules are installed it may not need all of these
		$params = array($userid, $userid, $userid, $userid, $userid, $userid, $userid, $userid, $userid, $userid, $userid);
		$rs = $remotedb->get_recordset_sql($sql, $params, $limitfrom=0, $limitnum=0);

		//Print titles for each column: Course, Assessment, Type, Due Date, Submission Date, Submission, Feedback, Grade, Highest Grade
		$newtext=	"<table class=\"grades\" id=\"grades\">
						<thead>
							<tr>
								<th>".get_string('gradetblheader_course', 'report_myfeedback')."</th>
								<th>".get_string('gradetblheader_assessment', 'report_myfeedback')."</th>
								<th>".get_string('gradetblheader_type', 'report_myfeedback')."</th>
								<th>".get_string('gradetblheader_duedate', 'report_myfeedback')."</th>
								<th>".get_string('gradetblheader_submissiondate', 'report_myfeedback')."</th>
								<th>".get_string('gradetblheader_submission', 'report_myfeedback')."</th>
								<th>".get_string('gradetblheader_feedback', 'report_myfeedback')."</th>
								<th>".get_string('gradetblheader_grade', 'report_myfeedback')."</th>
								<th>".get_string('gradetblheader_highestgrade', 'report_myfeedback')."</th>
							</tr>
						</thead>";
		...
		if ($rs->valid()) {
			...
			foreach ($rs as $record)
			{
				...
				$feedbacktext = "&nbsp;";
				$submission = "&nbsp;";
				$assignmentname =  "<a href=\"".$CFG->wwwroot."/mod/".$record->assessmenttype."/view.php?id=".$record->assignmentid."\">".$record->assessmentname."</a>";
				...
				if($record->assessmenttype){
					$assessmenttype = '<img src="' . $OUTPUT->pix_url('icon', $record->assessmenttype) . '" '.'class="icon" alt="' . $record->assessmenttype . '">';
					switch($record->assessmenttype){
						case "assign":
							$assessmenttype .= get_string('moodle_assignment', 'report_myfeedback');
							
							if($record->teamsubmission == 1){
									$assessmenttype .= " (".get_string('groupwork', 'report_myfeedback').")";
								if(!is_numeric($submissiondate) || (!strlen($submissiondate) == 10)){	
									$submissiondate = $this->get_group_assign_submission_date($userid, $record->contextid);
								}
							}
							...
							if($record->nosubmissions == 0){
								...
								if(is_numeric($submissiondate) && (strlen($submissiondate) == 10)){
									$submission = 	"<a href=\"".$CFG->wwwroot."/mod/".$record->assessmenttype."/view.php?id=".$record->assignmentid."&action=view\">".
														get_string('submission', 'report_myfeedback').
													"</a>";
								}	

								if($record->status == "draft"){
									$submission .= " (".get_string('draft', 'report_myfeedback').")";
									$submissiondate = "<i>".$submissiondate." (draft)</i>"; // this will set it to no submission later because it checks for all numbers
								}
							...
							if($record->assigngradeid){
								$onlinepdffeedback = $this->has_pdf_feedback_file($record->assigngradeid);
							}
							//if there are any comments or other feedback (such as online PDF files, rubrics or marking guides)
							//TODO:	Check whether rubrics / marking guide has been filled out		
							if($feedbacktext != "&nbsp;" || $record->feedbacklink || $onlinepdffeedback || $record->activemethod == "rubric" ||  $record->activemethod == "guide"){
								$feedbacktext = "<a href=\"".$CFG->wwwroot."/mod/assign/view.php?id=".$record->assignmentid."\">".
												get_string('feedback', 'report_myfeedback').
												"</a>";
							}
							break;
						case "turnitintool":
							...
							$newwindowicon = '<img src="' . 'pix/external-link.png' . '" '.' alt="-" title="'.$newwindowmsg.'" />';
							if($record->assignmentid && $record->subpart && $record->tiiobjid && $record->assessmentname){
								//not sure what utp is, but it seems to work when set to 2 for admin and 1 for students

								//link the submission to the plagiarism comparison report
								$submission = "<a href=\"".$CFG->wwwroot."/mod/turnitintool/view.php?id=".$record->assignmentid."&jumppage=report&userid=".$userid."&utp=1&partid=".$record->subpart."&objectid=".$record->tiiobjid."\" target=\"_blank\">".get_string('submission', 'report_myfeedback')."</a> $newwindowicon";
								
								//if grademark marking is enabled
								if($record->usegrademark == 1){
									//link the submission to the gradebook
									$feedbacktext = "<a href=\"".$CFG->wwwroot."/mod/turnitintool/view.php?id=".$record->assignmentid."&jumppage=grade&userid=".$userid."&utp=1&partid=".$record->subpart."&objectid=".$record->tiiobjid."\" target=\"_blank\">".get_string('feedback', 'report_myfeedback')."</a> $newwindowicon";;
								}else{
									$feedbacktext = "<a href=\"".$CFG->wwwroot."/mod/turnitintool/view.php?id=".$record->assignmentid."&do=submissions\">".
														get_string('feedback', 'report_myfeedback').
													"</a>";
								}	
							}
						...
						case "workshop":
							...
							$submission = "<a href=\"".$CFG->wwwroot."/mod/workshop/submission.php?cmid=".$record->assignmentid."&id=".$record->subid."\">".
												get_string('submission', 'report_myfeedback').
											"</a>";
							
							$feedbacktext = "<a href=\"".$CFG->wwwroot."/mod/workshop/submission.php?cmid=".$record->assignmentid."&id=".$record->subid."\">".
												get_string('feedback', 'report_myfeedback').
											"</a>";
							if($record->nosubmissions == 0){
								$submissiondate = get_string('offline_assignment', 'report_myfeedback');
						...
						case "quiz":
							...
							$submission = "-";
							$submissiondate = "-";
							$feedbacktext = $this->get_quiz_attempts_link($record->assignid, $userid, $record->assignmentid);
							break;
					}
				}
				...
				}elseif($submissiondate != get_string('offline_assignment', 'report_myfeedback') &&  strpos($assessmenttype,"quiz") ===FALSE){
				...
				if($submittedtime > $record->duedate && $submissiondate != get_string('offline_assignment', 'report_myfeedback')){
					if($submissionmsg == ""){
						$submissionmsg = get_string('late_submission_msg', 'report_myfeedback');
					}
					$alerticon = '<img src="' . 'pix/warning2-faded.png' . '" '.'class="icon" alt="-" title="'.$submissionmsg.'" />';
					$submissiondate = "<span style=\"color: red;\">".$submissiondate." $alerticon</span>";
				}

				$newtext.="<tr>";
					$newtext.="<td>".($record->coursename && $record->courseid ? 
									"<a href=\"".$CFG->wwwroot."/course/view.php?id=".$record->courseid."\">".
										$record->coursename.
									"</a>":"&nbsp;").
								"</td>";
	...
	/**
	* Tell Moodle the report can only be shown once - not sure if this is relevant to reports, or just blocks.
	* 
